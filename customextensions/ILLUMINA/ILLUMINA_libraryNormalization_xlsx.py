...
def visualization(f_out, analysis, limsID, sample) :
    ...
    lcol += 1
#    f_out.write_number(xrow, lcol, sample['ngPerSample'], cell_format_2dec)
#    lcol += 1
#    f_out.write_number(xrow, lcol, sample['dilutionFactor'], cell_format_2dec) 
...
#        sampleData[name] = []
        ...
        if "TWIST" in name:
            ...
            if fragmentLength == "" :
                nM = float(libraryConc) * nMConversionFactor[name]
            else:
...
def main():
    ...
    columns = (40, 22, 15, 25, 10, 20, 15, 10)
