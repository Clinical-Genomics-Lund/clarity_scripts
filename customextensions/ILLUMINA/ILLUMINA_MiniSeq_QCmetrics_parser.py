...
# Script is MiniSeq specific (4 lanes)
...
def pull_udf_values_into_lims(html_file, runDirectory):
    ...
    for udf, tableColumn in udfs_in_clarity.items() :
        if udf == "% PF Clusters" :
            ...
            cleanTable1[udf] = float(table1[tableColumn]) 
...
def main():
    ...
    if (found_directory == "" ) :
        print "Could not find a MiniSeq run with the provided reagent cartridge ID. Can not import run data. Run not finished yet?"
        sys.exit(255)
