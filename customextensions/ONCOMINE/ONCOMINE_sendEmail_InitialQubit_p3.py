...
def email_template1(eMailInfo ):
	...
	body.append( "Submitted ID_Lims ID\tQubitConc (ng/ul)\tStatus\tAnalysis\n")
...
def main():
	...
	for i in Inputs :
		...
		conc = api.getUDF( aDOM, "Qubit concentration (ng/ul)" )
		...
		if department == "Klinisk Patologi" and "Oncomine Focus" in analysis:
			eMailONC.append({"name" : name, "limsID" : limsID, "conc" : conc, "qc" : qc , "analysis" : analysis}) 
