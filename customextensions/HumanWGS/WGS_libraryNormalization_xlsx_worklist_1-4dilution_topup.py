...
def visualization(f_out, data) :
    ...
    lcol += 1
    f_out.write_string(xrow, lcol, data['limsID'])
    lcol += 1
    f_out.write_string(xrow, lcol, data['well'])
    lcol += 1
    f_out.write_number(xrow, lcol, data['volumePerSample'], cell_format_2dec)
    lcol += 1
    f_out.write_string(xrow, lcol, data['comment'])
    xrow += 1

def normalization(f_out, c_out, data, totalReadSum, loadingConc, loadingVolume) : 
    global xrow
    global xcol
    global cell_format_yellow
    global cell_format_blue
    lcol = 0
    for i in ['LabID', 'LimsID', 'WellLocation', 'VolumePerSample(ul) from 1:4 dilution', 'Comment'] :
        f_out.write(xrow, lcol, i)
        lcol += 1
    xrow += 1

    c_out.write('LabID,LimsID,Source Plate,WellLocation,Pool Tube,Dest Well,VolumePerSample(ul) from 1:4 dilution,Comment\n')
	...
	tecanWell = data[ k ][ 'well' ].split( ':' )
        if int(tecanWell[1]) > 3:
            tube = 2
        else:
            tube = 1
	# Add 6 to column number to take from 1:4 dilution
	tecanWell = tecanWell[0] + str(int(tecanWell[1]) + 6)

        # Visualization to user
        visualization(f_out, data[k])
	c_out.write(','.join([data[k]['labID'], data[k]['limsID'], 'SamplePlate[001]', tecanWell, 'Pool[00%d]' % tube, 'A1', '%.2f' % data[k]['volumePerSample'], data[k]['comment']]) + '\n')

    bufferVol = poolVolume - sumOfSampleVolumes
    pooledLibraryVolume = poolVolume - bufferVol

    # Add buffer manually instead
    #c_out.write(',,Buffer,A1,%.2f,Pool,A1,%.2f,\n' % (bufferVol, bufferVol))
...
def getSampleData(stepURI, instrument, availableReads, kit ) :
    ...
    for limsid in CACHE_IDS :
        ...
        container = aDOM.getElementsByTagName( "container" )[0].getAttribute("limsid")
        well = aDOM.getElementsByTagName( "value" )[0].firstChild.data
        sortKey = container + "_" + well.split(':')[1] + well.split(':')[0]
        ...
        topup = api.getUDF( sDOM, "Topup read count" )
        if topup != "": 
            #Use topup read count as read count
            readCount = topup
            comment = "topup" 
        else : 
            comment = ""
...
def main():
    ...
    args = {}
    opts, extraparams = getopt.getopt(sys.argv[1:], "s:u:p:c:k:l:f:")
    for o,p in opts:
        if o == '-s':
            args[ "stepURI" ] = p
        elif o == '-u':
    ...
    c_outfile = open(args[ "csvOut" ] + '-pool-worklist.csv', 'w')

    normalization(f_outsheet, c_outfile, sampleData, totalReadSum, loadingConc, loadingVolume)
