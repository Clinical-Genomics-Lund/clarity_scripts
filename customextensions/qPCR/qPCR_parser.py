...
def parseinputFile(DilutionFactor):
    ...
    for row in data[Header:]:
        ...
        for column, UDF in artifactUDFMap.items():
            ...
            if column == 'Quantity Mean' :
                try : 
                    UDFresults[ UDF ] = float ( values[ columnHeaders.index( column )].replace(',', '.' ) )* DilutionFactor
                except : 
                    pass
...
def limslogic():
    ...
    for udf in stepdetails.getElementsByTagName( "udf:field" ):
        if udf.getAttribute( "name" ) == "Dilution Factor":
            DilutionFactor = int( udf.firstChild.data )

    artifactUDFResults = parseinputFile(DilutionFactor)
    ...
    r = api.POST( output_artifacts.toxml().encode('utf-8'), api.getBaseURI() + "artifacts/batch/update" )
