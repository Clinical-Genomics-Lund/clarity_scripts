...
def POSTPlacementValues( stepdetails, PlateID, dest_containerLimsID ,  oArtifacts, Result_Artifacts ):
    ...
    for oArt in oArtifacts:
        placeXML.append( '<output-placement uri="' + oArtifacts[oArt] + '"><location><container uri="' + api.getBaseURI() + 'containers/' + dest_containerLimsID + '" limsid="' + dest_containerLimsID + '"/>' )
...
def main():
    ...
    for oArt in oArtifacts:
        oDOM = parseString( api.GET(oArtifacts[oArt]) )
        oDOM = api.setUDF(oDOM, "QS validity of result" , Result_Artifacts[oArt][1] )
        r = api.PUT( oDOM.toxml().encode('utf-8'), oArtifacts[oArt])
        logging.info(r)
