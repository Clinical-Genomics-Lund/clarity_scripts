...
def doPOST(paURI, arts):
        ...
        for art in arts:
            ...
            if ( (LibraryType == 'DNA') or (LibraryType == 'RNA' ) ) : 
                ...
                print "The sample name must be tagged with CL, CHP or CF"
                sys.exit(255)

        if (len(cancer_samples) > 0 ) :
            cancerXML = '<rt:routing xmlns:rt="http://genologics.com/ri/routing"><assign stage-uri="https://mtapp046.lund.skane.se/api/v2/configuration/workflows/251/stages/401">'
            for sample in cancer_samples:
                cancerXML = cancerXML + '<artifact uri="' + sample + '"/>'
            cancerXML = cancerXML + '</assign>'
            cancerXML = cancerXML + '<unassign workflow-uri="https://mtapp046.lund.skane.se/api/v2/configuration/workflows/251"><artifact uri="' + paURI + '"/></unassign></rt:routing>'
            response_cancer = api.POST( cancerXML, BASE_URI + "route/artifacts/" )
            f.write('\nRouted cancer sample:\n' + cancerXML+ '\n')
                 
        if (len(CF_samples) > 0 ) :
            CFXML = '<rt:routing xmlns:rt="http://genologics.com/ri/routing"><assign stage-uri="https://mtapp046.lund.skane.se/api/v2/configuration/workflows/301/stages/415">'
            for sample in CF_samples:
                CFXML = CFXML + '<artifact uri="' + sample + '"/>'
            CFXML = CFXML + '</assign>'
            CFXML = CFXML + '<unassign workflow-uri="https://mtapp046.lund.skane.se/api/v2/configuration/workflows/301"><artifact uri="' + paURI + '"/></unassign></rt:routing>'
