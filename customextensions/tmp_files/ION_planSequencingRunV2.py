...
def main():
    ...
    if InputAnalytes_DNA :
        ...
        f.write("Sample Name\tWell location\tSample LimsID\tUndiluted library (ul)\t1:25 Diluted library (ul)\n")
        for sample in InputAnalytes_DNA :
            f.write(sample[sample.keys()[0]]["name"] + "\t" + sample[sample.keys()[0]]["well"] + "\t" + sample.keys()[0] + "\t" + format(sample[sample.keys()[0]]["LibraryVolume"]) + "\t" + format(sample[sample.keys()[0]]["DilutedLibraryVolume"]) + "\n")
