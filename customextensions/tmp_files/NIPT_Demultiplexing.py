...
def doPOST(paURI, arts):
    ...
    NIPTXML = '<rt:routing xmlns:rt="http://genologics.com/ri/routing"><assign stage-uri="https://mtapp046.lund.skane.se/api/v2/configuration/workflows/151/stages/263">'
    for art in arts:
        NIPTXML = NIPTXML + '<artifact uri="' + art + '"/>'
    NIPTXML = NIPTXML + '</assign>'
    NIPTXML = NIPTXML + '<unassign workflow-uri="https://mtapp046.lund.skane.se/api/v2/configuration/workflows/151"><artifact uri="' + paURI + '"/></unassign></rt:routing>'
